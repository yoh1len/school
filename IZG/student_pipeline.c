...
if(indices == NULL){
gl_VertexID = vertexShaderInvocation;
}
else{
gl_VertexID = indices[vertexShaderInvocation];
}
  return gl_VertexID;
}
...
const int *vertex_pointer = head->buffer + head->offset + gl_VertexID*head->stride;
...
VertexIndex GLVertexID = gpu_computeGLVertexID(puller->indices, vertexShaderInvocation);

int i = 0;
while(i< MAX_ATTRIBUTES){
GPUVertexPullerHead const *head = &puller->heads[i];
if(head->enabled == 1){
output->attributes[i] = gpu_computeVertexAttributeDataPointer(head, GLVertexID);
}
else{
...
primitive->nofUsedVertices = nofPrimitiveVertices; //test
int i = 0;
...
float x1 = vertices[0].data[0];
float x2 = vertices[1].data[0];
float x3 = vertices[2].data[0];
float y1 = vertices[0].data[1];
float y2 = vertices[1].data[1];
float y3 = vertices[2].data[1];
float x = pixelCenter[0].data[0];
float y = pixelCenter[0].data[1];

float delitel = (y2-y3)*(x1-x3)+(x3-x2)*(y1-y3);

float lam1 = ((y2-y3)*(x-x3)+(x3-x2)*(y-y3))/delitel;
float lam2 = ((y3-y1)*(x-x3)+(x1-x3)*(y-y3))/delitel;
float lam3 = 1-lam1-lam2;

init_Vec3(coords, lam1,lam2,lam3);
}
